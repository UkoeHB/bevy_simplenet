...
#[derive(Debug)]
pub struct TokioPendingResult<R>
{
    join_handle: Option<tokio::task::JoinHandle<R>>
}

impl<R> TokioPendingResult<R>
{
    /// Make a new pending result.
    pub fn new(join_handle: tokio::task::JoinHandle<R>) -> TokioPendingResult<R>
    {
        TokioPendingResult{ join_handle: Some(join_handle) }
    }

    /// Make a pending result that is immediately ready.
    pub fn immediate(result: R, runtime: &tokio::runtime::Runtime) -> TokioPendingResult<R>
    where
        R: Send + 'static
    {
        TokioPendingResult::<R>::new(runtime.spawn(futures::future::ready::<R>(result)))
    }
    ...
    /// - This is robust for checking if a result-less task has completed (i.e. `TokioPendingResult<()>`).
